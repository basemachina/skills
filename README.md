...
`basemachina-docs` は BaseMachina の仕様確認、`bm-code-management` は BaseMachina のコード管理 repo を編集するときのガードレールです。`bm-code-management` はアクションの実行、本番反映、ビュー設定そのものの変更は扱いません。
